...
extern double* ship_coor_x; // holds the x coordinates
extern double* ship_coor_y; // holds the y coordinates
extern int num_coor;
extern long gravity;
extern int bounds_check;

// our function which we will have called whenever the timer expires
void handle_timeout(int signal);
void block_signal(FILE* executable, double* ship_coor_x, double* ship_coor_y, int num_coor, double gravity);
...
void handle_timeout(int signal)
{
    ...
    if (signal == SIGALRM)
    {
        ...
        block_signal(executable, ship_coor_x, ship_coor_y, num_coor, gravity);
        //--------------------------------------------------------------
...
void block_signal(FILE* executable, double* ship_coor_x, double* ship_coor_y, int num_coor, double gravity)
{
    ...
    static double the_new_y = 0.0;
    static double the_old_y = 0.0;
    static double the_old_velocity = 0.0;
    static double t = 0.5;
    static double the_new_velocity = 0.0;
    ...
    for (k=0; k<num_coor; k++) {
        ship_coor_x[k] += the_new_velocity;
        ship_coor_y[k] += the_new_velocity;
    } 
    ...
    for(k = 1 ; k < num_coor; k++) {
        ...
        x1_r = round(ship_coor_x[k-1]);
        y1_r = round(ship_coor_y[k-1]);
        x2_r = round(ship_coor_x[k]);
        y2_r = round(ship_coor_y[k]);
